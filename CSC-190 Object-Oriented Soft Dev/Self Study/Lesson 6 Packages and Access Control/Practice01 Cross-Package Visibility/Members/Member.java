...
public class Member {
    ...
    protected Book[] borrowedBooks = new Book[5];
    ...
    public void borrowBook(Book book) {
        ...
        if (book.isAvailable) { // boolean default return is true, '!' if false.
            ...
            if (BookCount == borrowedBooks.length) {
                System.out.println(Name+" (Regular Member) has hit the limit to borrow books. [5/5]");
                return; // stops the whole method here
            }
            //? CHECK DUPLICATION NOTIFICATION
            for (int i = 0; i < BookCount; i++) {
                if (book == borrowedBooks[i]) { // if it matches
                    System.out.println(Name+" (Regular Member) is already borrowing "+book.getTitle()+" ("+book.getAuthor()+")");
                    return; // stops the whole method here
                }
            }
            //? SETTING UP
            ...
            BookCount++;
            System.out.println(Name+" (Regular Member) has borrowed "+book.getTitle()+" ("+book.getAuthor()+")");
